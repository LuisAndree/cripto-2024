def gerar_matriz(chave):
    alfabeto = "abcdefghiklmnopqrstuvwxyz"  # 'j' é geralmente omitido
    matriz = []
    letras_usadas = set()
...
def cifra_playfair(mensagem, chave, modo='cifrar'):
    ...
    while i < len(mensagem):
        ...
        if linha_a == linha_b:
            coluna_a = (coluna_a - 1) % 5 if modo == 'decifrar' else (coluna_a + 1) % 5
            coluna_b = (coluna_b - 1) % 5 if modo == 'decifrar' else (coluna_b + 1) % 5
        elif coluna_a == coluna_b:
            linha_a = (linha_a - 1) % 5 if modo == 'decifrar' else (linha_a + 1) % 5
            linha_b = (linha_b - 1) % 5 if modo == 'decifrar' else (linha_b + 1) % 5
        else:
            coluna_a, coluna_b = coluna_b, coluna_a

        mensagem_decifrada += matriz[linha_a][coluna_a] + matriz[linha_b][coluna_b]
        i += 2
...
if __name__ == "__main__":
    ...
    mensagem_decifrada = cifra_playfair(mensagem_cifrada, chave, modo='decifrar')
